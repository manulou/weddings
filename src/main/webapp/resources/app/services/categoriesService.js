...
var CategoriesService = (function () {
    function CategoriesService(_http, configuration) {
        var _this = this;
        this._http = _http;
        this.configuration = configuration;
        this.getAll = function () {
            return _this._http.get(_this.actionUrl + 'getCategories')
                .map(function (response) { return response.json(); }).catch(_this.handleError);
        };
        this.save = function (category) {
            return _this._http.post(_this.configuration.SecureServerWithApiUrl + 'saveCategory', category)
                .map(function (response) { return response.json(); }).catch(_this.handleError);
        };
